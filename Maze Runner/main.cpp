...
int main(void)
{
	...
	ALLEGRO_FONT* font20 = al_load_ttf_font("GROBOLD.ttf", 15, 0);
	...
	while(!gameOver)
	{
		...
		if (levelOver) { //loads next level map and sprite
			levelOver = false;
			level++;
			if (level == 2)
			{
			...
			{
				cout << "CONGRATULATIONS!! YOU BEAT ALL OF THE LEVELS. PLAY AGAIN?";
				al_rest(5);
				gameOver = true;
				break;
			}
		...
		if (ev.type == ALLEGRO_EVENT_TIMER)
		{
			...
			if (player.CollisionEndBlock()) {
				cout << "CONGRATS, YOU BEAT THIS LEVEL!\n";
				al_rest(3);
				levelOver = true;
			}
